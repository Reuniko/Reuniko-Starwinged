Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro Fur/8391af6eaf6b9284299eba44fa7fe10b"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void GeometryProgram(triangle VertexOut p[3], inout TriangleStream <VertexOut> tristream, uint InstanceID : SV_GSInstanceID)
			{
				...
				for (int i = 0; i < 3; i ++)
				{
					...
					float3 furGravityOffset = (float3(0, -1, 0) * (furSettings.y * (0.01 /*_FurLength*/)) *(0.0 /*_FurGravityStrength*/)) * smoothstep(-0.5, 1, dot(normal, float3(0, -sign((0.0 /*_FurGravityStrength*/)), 0)) * 0.5 + 0.5);
					t.furSettings = furSettings;
					float3 offset = furGravityOffset + furSettings.y * (0.01 /*_FurLength*/) * normal;
			...
			{
				float furMask = POI2D_SAMPLER_PAN(_FurMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_FurMaskUV*/)], float4(50,50,0,0)), float4(0,0.5,0,0)).x;
				float furHeightMap = POI2D_SAMPLER_PAN(_FurHeightMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_FurHeightMapUV*/)], float4(100,100,0,0)), float4(0,0,0,0)).x;
				furHeightMap = saturate(remap(furHeightMap, 0, float4(0,1,0,1).y, float4(0,1,0,1).x, 1));
				float ao = lerp(1, smoothstep(float4(0,1,0,1).x, float4(0,1,0,1).y, furHeightMap), (0.0 /*_FurAO*/) * furMask);
				float4 furTexture = POI2D_SAMPLER_PAN(_FurTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_FurTextureUV*/)], float4(1,1,0,0)), float4(0,0,0,0));
				furTexture.rgb *= poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_FurColorThemeIndex*/));
				if ((1.0 /*_FurHueShiftEnabled*/))
				{
					furTexture.rgb = hueShift(furTexture.rgb, _FurHueShift + _Time.x * (0.0 /*_FurHueShiftSpeed*/));
				}
				furTexture.rgb *= ao;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				#else
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#ifdef _RIMSTYLE_POIYOMI
				#if defined(PROP_RIMMASK) || !defined(OPTIMIZER_ENABLED)
				float rimMask = POI2D_SAMPLER_PAN(_RimMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_RimMaskUV*/)], float4(1,1,0,0)), float4(0,0,0,0))[(0.0 /*_RimMaskChannel*/)];
				#else
				float rimMask = 1;
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
